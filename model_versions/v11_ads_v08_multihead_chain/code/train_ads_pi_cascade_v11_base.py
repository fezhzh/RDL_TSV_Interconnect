# -*- coding: utf-8 -*-
"""Shared ADS cascade helpers for the v11 long-chain model.
...
optimization -> pi-parameter NN -> S-parameter fine-tuning.
"""
...
RDL_ADS_SIM_DIR = PROJECT_ROOT / "model_versions" / "v11_ads_v08_multihead_chain" / "rdl_ads_sim"
TSV_ADS_SIM_DIR = PROJECT_ROOT / "model_versions" / "v11_ads_v08_multihead_chain" / "tsv_ads_sim"
...
RDL_ADS = load_ads_helper("v11_rdl_ads_sim", RDL_ADS_SIM_DIR / "ADS_Sim.py")
TSV_ADS = load_ads_helper("v11_tsv_ads_sim", TSV_ADS_SIM_DIR / "ADS_Sim.py")


# Direct-run configuration.
RUN_LABEL = "v11_ads_pi_cascade_default"
OUTPUT_DIR = PROJECT_ROOT / "model_versions" / "v11_ads_v08_multihead_chain" / "results" / RUN_LABEL
...
class AdsSimulationRunner:
    ...
    def simulate(self, device_name: str, sample_id: str, row: pd.Series) -> rf.Network:
        ...
        out_s2p = out_base.with_suffix(".s2p")
        if out_s2p.exists() and out_s2p.stat().st_size > 0:
    ...
    if SIMULATION_BACKEND == "development_cached_snp":
        ...
        return SimulationBundle(
            ...
            simulator_report={
                ...
                "note": "Used current v09/v03 single-device models to smoke-test the ADS pi workflow.",
    ...
    torch.save(
        {
            ...
            "metadata": {
                "model_type": "v11_ads_pi_cascade_base",
                "stage": stage,
...
def run_once(settings: dict[str, float]):
    ...
    report = {
        ...
        "workflow": [
            f"ADS single-device S-parameters are cascaded and each sample optimizes {CONNECTION_COUNT} pi-network element sets.",
    ...
    (OUTPUT_DIR / "validation_archive.md").write_text(
        "\n".join(
            [
                ...
                "## Workflow",
                "",
                f"1. Optimize {CONNECTION_COUNT} pi-network element sets for each cascaded ADS sample.",
