# -*- coding: utf-8 -*-
"""Train an RDL single-device model for LHS400_Connection2.

Run this file directly in VS Code after `extract_rdl_connection2_params.py`.
...
PARAM_CSV = (
    ...
    / "results"
    / "rdl_connection2_extracted_params"
    / "RDL_connection2_circuit_params.csv"
)
OUTPUT_DIR = PROJECT_ROOT / "model_versions" / "v12_hfss_v08_multihead_chain" / "results" / "rdl_connection2_sparam_model"
...
class RdlParamNet(nn.Module):
    def __init__(self, input_dim: int = 4, output_dim: int = 9):
        super().__init__()
...
def train_model(model, df, arrays, device):
    ...
    length_val = torch.tensor(df.loc[val_mask, LENGTH_COLUMN].to_numpy(dtype=np.float64), dtype=REAL_DTYPE, device=device)
    ...
    train_ds = TensorDataset(
        ...
        torch.tensor(df.loc[train_mask, LENGTH_COLUMN].to_numpy(dtype=np.float64), dtype=REAL_DTYPE),
...
def evaluate(model, df, arrays, device):
    ...
    for i, row in df.iterrows():
        pred_s = circuit_params_to_s_np(pred_params[i], row[LENGTH_COLUMN], freq)
        target = s_target[i]
...
def save_plots(metrics: pd.DataFrame, df: pd.DataFrame, pred_df: pd.DataFrame, s_target: np.ndarray, freq: np.ndarray):
    ...
    for group, sub in selected:
        for _, metric in sub.iterrows():
            ...
            pred_s = circuit_params_to_s_np(pred_params, df.loc[i, LENGTH_COLUMN], freq)
            target = s_target[i]
            fig, axes = plt.subplots(2, 2, figsize=(12, 8), dpi=150)
            fig.suptitle(f"RDL Connection2 dut{int(metric['dut_index'])} | {group}", x=0.02, y=0.98, ha="left")
            specs = [
            ...
            for ax, (title, fn) in zip(axes.ravel(), specs):
                ...
                ax.plot(freq / 1e9, fn(pred_s), label="New RDL model", color="#dc2626", linestyle="--", linewidth=1.5)
                ax.set_title(title)
                ax.set_xlabel("Frequency (GHz)")
                ax.grid(True, alpha=0.3)
                ax.legend(fontsize=8)
            out = plot_dir / group / f"RDL_connection2_dut{int(metric['dut_index'])}_{group}.png"
...
def main() -> None:
    ...
    if not PARAM_CSV.exists():
        raise FileNotFoundError(f"Missing extracted RDL parameter CSV: {PARAM_CSV}")
    ...
    model = RdlParamNet().to(dtype=REAL_DTYPE, device=device)
    ...
    torch.save({"model_state_dict": model.state_dict(), "metadata": arrays[7]}, OUTPUT_DIR / "rdl_connection2_sparam_net.pt")
    history.to_csv(OUTPUT_DIR / "rdl_connection2_training_history.csv", index=False, encoding="utf-8-sig")
    metrics.to_csv(OUTPUT_DIR / "rdl_connection2_sparam_metrics.csv", index=False, encoding="utf-8-sig")
    pred_df.to_csv(OUTPUT_DIR / "rdl_connection2_param_predictions.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(OUTPUT_DIR / "rdl_connection2_sparam_summary.csv", index=False, encoding="utf-8-sig")
    report = {
    ...
    (OUTPUT_DIR / "validation_archive.md").write_text(
        "\n".join(
            [
                "# v12 RDL Connection2 S-Parameter Model Training",
                "",
                f"- Entry: `{Path(__file__).name}`",
                f"- Parameter CSV: `{PARAM_CSV}`",
                f"- Output: `{OUTPUT_DIR}`",
                f"- Model: `{OUTPUT_DIR / 'rdl_connection2_sparam_net.pt'}`",
                f"- Plots: `{plot_dir}`",
                "",
                "## Summary",
                "",
