# -*- coding: utf-8 -*-
"""Extract RDL equivalent-circuit parameters for LHS400_Connection2.

Run this file directly in VS Code. No command-line arguments are required.
The output is version-local under v12 results and does not overwrite older RDL
parameter datasets.
"""
...
INPUT_DIR = PROJECT_ROOT / "HFSS_sim" / "LHS400_Connection2" / "train" / "RDL"
VARIATION_CSV = PROJECT_ROOT / "HFSS_sim" / "LHS400_Connection2" / "train" / "RDL_variations_record.csv"
OUTPUT_DIR = PROJECT_ROOT / "model_versions" / "v12_hfss_v08_multihead_chain" / "results" / "rdl_connection2_extracted_params"
OUTPUT_CSV = OUTPUT_DIR / "RDL_connection2_circuit_params.csv"
...
def extract_one(path: Path, dut_index_from_name: int, variation_row: pd.Series | None) -> list[object]:
    ...
    if variation_row is not None:
        ...
        l_tmrdl = float(variation_row["l_tmrdl"])
        w_tmrdl = float(variation_row["w_tmrdl"])
        h_tmrdl = float(variation_row["h_tmrdl"])
        dut_index = int(dut_index_from_name)
    else:
        pitch = float(variables["pitch"])
        l_tmrdl = float(variables["l_tmrdl"])
        w_tmrdl = float(variables["w_tmrdl"])
        h_tmrdl = float(variables.get("h_tmrdl", variables.get("t_tmrdl")))
...
def main() -> None:
    ...
    variation_df = pd.read_csv(VARIATION_CSV, encoding="utf-8-sig").rename(columns={"t_tmrdl": "h_tmrdl"}).set_index("dut_index")
    rows = []
    failures = []
    for i, path in enumerate(sorted(INPUT_DIR.glob("dut*.s2p"), key=lambda p: int(re.search(r"dut(\d+)", p.stem).group(1))), start=1):
        ...
        if i == 1 or i % 50 == 0:
            print(f"extracted {i}/{len(list(INPUT_DIR.glob('dut*.s2p')))} RDL Connection2 files", flush=True)

    out = pd.DataFrame(rows, columns=CSV_HEADERS).sort_values("dut_index").reset_index(drop=True)
    out.to_csv(OUTPUT_CSV, index=False, encoding="utf-8-sig")
    failure_df = pd.DataFrame(failures)
    failure_csv = OUTPUT_DIR / "RDL_connection2_extraction_failures.csv"
    ...
    summary = {
        ...
        "feature_columns": ["pitch", "l_tmrdl", "w_tmrdl", "h_tmrdl"],
        "length_column": "l_tmrdl",
        "pitch_min": float(out["pitch"].min()) if len(out) else None,
        "pitch_max": float(out["pitch"].max()) if len(out) else None,
        "l_tmrdl_min": float(out["l_tmrdl"].min()) if len(out) else None,
        "l_tmrdl_max": float(out["l_tmrdl"].max()) if len(out) else None,
    ...
    (OUTPUT_DIR / "validation_archive.md").write_text(
        "\n".join(
            [
                "# v12 RDL Connection2 Parameter Extraction",
                "",
