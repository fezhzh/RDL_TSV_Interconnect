# -*- coding: utf-8 -*-
"""Batch HFSS simulations for the 400-sample TSV/RDL LHS dataset.
...
BASE_DATA_OUT_PATH = WORK_DIR / "LHS400"
DATA_OUT_PATH = WORK_DIR / "LHS400"
BASE_SPLIT_NAME = "train"
INCREMENTAL_AUGMENT_MODE = False
CONTINUE_DUT_INDEX_FROM_PREVIOUS_LHS = True
FIXED_DUT_INDEX_START = 300
PREVIOUS_LHS_DATA_PATHS = [
    WORK_DIR / "LHS100",
    WORK_DIR / "LHS200",
]
...
def previous_lhs_record_file(data_path, design_name):
    return Path(data_path) / BASE_SPLIT_NAME / f"{design_name}_variations_record.csv"


def previous_lhs_row_count(design_name):
    total = 0
    for data_path in PREVIOUS_LHS_DATA_PATHS:
        record_file = previous_lhs_record_file(data_path, design_name)
        if not record_file.exists():
            print(f"No previous LHS record found for {design_name}: {record_file}")
            continue
        rows = read_variation_record(record_file, design_name, validate_pitch=False)
        total += len(rows)
        print(f"Loaded {len(rows)} previous LHS rows for {design_name}: {record_file}")
    return total


def assign_continued_dut_indices(design_name, rows):
    if FIXED_DUT_INDEX_START is not None:
        start_idx = int(FIXED_DUT_INDEX_START)
    else:
        start_idx = previous_lhs_row_count(design_name)
    for local_idx, row in enumerate(rows):
        row["dut_index"] = start_idx + local_idx
    print(f"{design_name}: assigned dut_index {start_idx} to {start_idx + len(rows) - 1}")
    return rows
        ...
        for design_name in TARGET_DESIGNS:
            ...
            if record_file.exists():
                ...
                if INCREMENTAL_AUGMENT_MODE:
                    ...
                    if CONTINUE_DUT_INDEX_FROM_PREVIOUS_LHS:
                        rows = assign_continued_dut_indices(design_name, rows)
